---
title: Alert
---

import Image from "next/image";

# Button Variants

The `Alert` components in **sidcn** is your go-to solution for displaying attention-grabbing alerts in your React Native and Expo applications. With a variety of variants to choose from, it seamlessly integrates into your app, making sure your users stay informed and engaged.




<div style={{ display: "flex", flexDirection: "row",marginTop:40}}>
  <Image
    src="/assets/components/Alert/Frame.svg"
    alt="Hello"
    width={250}
    height={250}
    style={{marginLeft:'auto',marginRight:10}}
  />
  <Image
    src="/assets/components/Alert/Frame-1.svg"
    alt="Hello"
    width={250}
    height={250}
    style={{marginRight:'auto',marginLeft:10}}

/>


</div>

## Import Alert

The primary variant is designed for the main action or call-to-action buttons.

```jsx copy
import { Alert, AlertTitle, AlertDescription } from 'sidcn';
```
### How to use

You can have Icon | Image | View | Text | Svg Component for Icon Purpose, Alert Title and Alert Description

```jsx copy
<Alert variant="primary">
  <Image
    source={require('../assets/favicon.png')}
    style={{ width: 24, height: 24 }}
  />
  <AlertTitle>Alert Primary</AlertTitle>
  <AlertDescription>Alert Description</AlertDescription>
</Alert>
```

`Use with className`

```jsx copy
<Alert variant="primary">
  <AlertTitle className="text-[20px]">Alert Primary</AlertTitle>
  <AlertDescription className="text-[20px]">Alert Description</AlertDescription>
</Alert>
```

### Primary Variant

The primary variant is designed for the main action or call-to-action buttons.

```jsx copy
<Alert variant="primary">
  <AlertTitle>Alert Primary</AlertTitle>
  <AlertDescription>Alert Description</AlertDescription>
</Alert>
```


### Secondary Variant

The secondary variant is suitable for secondary actions or buttons with less emphasis.

```jsx copy
<Alert variant="secondary">
  <AlertTitle>Alert Secondary</AlertTitle>
  <AlertDescription>Alert Description</AlertDescription>
</Alert>
```


### Destructive Variant

The destructive variant is ideal for buttons indicating potentially harmful actions.

```jsx copy
<Alert variant="destructive">
  <AlertTitle>Alert Destructive</AlertTitle>
  <AlertDescription>Alert Description</AlertDescription>
</Alert>
```


### Outline Variant

The outline variant provides a button with a transparent background and a visible border.

```jsx copy
 <Alert variant="outline">
  <AlertTitle>Alert Outline</AlertTitle>
  <AlertDescription>Alert Description</AlertDescription>
</Alert>
```

